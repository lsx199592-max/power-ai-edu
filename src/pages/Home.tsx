...
import { useCategories, useCourses, useAdvertisements } from '../hooks/useSupabaseData';
...
const Home: React.FC = () => {
  ...
  const { ads: topAds } = useAdvertisements('home_top');
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 py-12 sm:py-16 lg:py-32">
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20" />
        <div className="container mx-auto px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-base sm:text-xl text-slate-300 mb-6 sm:mb-8">从电力巡检到智能调度，掌握前沿 AI 技术，开启职业新篇章</p>
            <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4">
              <Link to="/courses" className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-lg transition-all transform hover:scale-105">
                <Play className="w-4 h-4 sm:w-5 sm:h-5" />开始学习
              </Link>
              <Link to="/courses" className="inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-lg border border-white/20 transition-all">
                查看课程<ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" />
              </Link>
            </div>
          </motion.div>
        </div>
      ...
      {topAds.length > 0 && (
        <section className="py-4 sm:py-6 bg-slate-800/50">
          <div className="container mx-auto px-4">
            <div className="relative overflow-hidden rounded-xl cursor-pointer" onClick={() => { const url = topAds[0].link_url; if (url?.startsWith('http')) window.open(url, '_blank'); else window.location.hash = url || ''; }}>
              <img src={topAds[0].image_url} alt={topAds[0].title} className="w-full h-32 sm:h-48 object-cover" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4 sm:p-6">
                <div>
                  <h3 className="text-white font-bold text-lg sm:text-xl">{topAds[0].title}</h3>
                  <p className="text-white/70 text-sm">{topAds[0].description}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                ...
                <Link to={`/courses/${course.slug}`} className="block group">
                  <div className="bg-slate-700/50 rounded-xl overflow-hidden border border-slate-600 hover:border-blue-500 transition-all">
                    <div className="aspect-video bg-slate-600 overflow-hidden">
                      <img src={course.cover_image || 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800'} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                    </div>
                    <div className="p-5">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1 text-yellow-400"><span className="text-sm">★</span><span className="text-sm text-white">{course.rating}</span></div>
            ...
            {topFeatured.length === 0 && (
              <div className="col-span-3 text-center py-12 text-slate-400">暂无精选课程，请在管理后台添加课程数据</div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="py-8 sm:py-16 bg-slate-800">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-gradient-to-br from-cyan-900/50 to-blue-900/50 rounded-2xl p-6 sm:p-8 border border-cyan-500/20">
              ...
              <Link to="/resources" className="inline-flex items-center gap-2 text-sm sm:text-base text-cyan-400 hover:text-cyan-300 font-medium">浏览资源 <ChevronRight className="w-4 h-4" /></Link>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-2xl p-6 sm:p-8 border border-purple-500/20">
              ...
              <Link to="/membership" className="inline-flex items-center gap-2 text-sm sm:text-base text-purple-400 hover:text-purple-300 font-medium">查看详情 <ChevronRight className="w-4 h-4" /></Link>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 sm:py-16 bg-slate-900">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-2xl p-6 sm:p-8 md:p-12 text-center">
            ...
            <Link to="/enterprise" className="inline-flex items-center gap-2 bg-white text-blue-600 font-semibold px-6 sm:px-8 py-2 sm:py-3 rounded-lg hover:bg-blue-50 transition-colors text-sm sm:text-base">立即咨询 <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" /></Link>
          </motion.div>
        </div>
      </section>
    </div>
